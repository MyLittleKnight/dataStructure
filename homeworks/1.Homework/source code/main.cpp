...
int main(int argc, char * argv[])
{
	...
	for(int i=0;i<sTotal-2;i++)
	{
		...
		if(errorType == 3)
		{
			cout<<"BORDER ERROR: Operator "<<action[i].type<<" with size "<<action[i].size<<" can not be placed on ("
				<<action[i].loc[0]<<","<<action[i].loc[1]<<")."<<endl;
			cout<<"/CONFLICT ERROR: Operator "<<action[i].type<<" with size "<<action[i].size<<" can not be placed on ("
				<<action[i].loc[0]<<","<<action[i].loc[1]<<")."<<endl;
		}
		else if(errorType==2)
		{
		...
		{
			cout<<"SUCCESS: Operator "<<action[i].type<<" with  size "<<action[i].size<<" is placed on ("
				<<action[i].loc[0]<<","<<action[i].loc[1]<<")."<<endl;
